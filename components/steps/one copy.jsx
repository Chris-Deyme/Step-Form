...
export const StepOne = ({ onNext, formData, setFormData }) => {
  ...
  const addStudent = (e) => {
    ...
    setStudents((prevStudents) => [...prevStudents, { disponibilites: [] }]);
  ...
  const addDisponibilite = (index, jour, debut, fin) => {
    ...
    // Vérifier s'il y a des conflits avec les disponibilités existantes
    const conflits = students[index].disponibilites.some((dispo) => {
    ...
    // Vérifier si la disponibilité existe déjà dans la liste des disponibilités de l'élève
    const existingDisponibilite = students[index]?.disponibilites.find(
      (dispo) =>
        dispo.jour === jour && dispo.debut === debut && dispo.fin === fin
    );

    // Si la disponibilité existe déjà, afficher un message d'erreur et empêcher l'ajout
    if (existingDisponibilite) {
      setDisponibiliteError("Cette disponibilité existe déjà.");
      return;
    }

    // Sinon, ajouter la nouvelle disponibilité à la liste des disponibilités de l'élève
    ...
    setStudents((prevStudents) => {
      ...
      if (updatedStudents[index]) {
        updatedStudents[index] = {
          ...updatedStudents[index],
          disponibilites: [
            ...(updatedStudents[index].disponibilites || []),
    ...
    // Effacer l'erreur après une saisie correcte
    setDisponibiliteError("");
  };
  ...
  const removeDisponibilite = (index, dispoIndex) => {
    setStudents((prevStudents) => {
      ...
      updatedStudents[index] = {
        ...updatedStudents[index],
        disponibilites: updatedStudents[index].disponibilites.filter(
  ...
  return (
    ...
    >
      <div className="p-4 bg-white rounded-lg md:w-4/5 lg:w-1/2 xl:w-1/2 w-4/5 mx-auto">
        ...
        <div class="grid gap-6 m-6 md:grid-cols-1">
